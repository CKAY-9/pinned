...
use crate::{ dto::Message, posts::dto::{ NewPostDTO, NewPostOTD } };
use actix_web::{ post, web, HttpRequest, HttpResponse, Responder };
use diesel::{ ExpressionMethods, QueryDsl, QueryResult, RunQueryDsl };
use pinned_db::create_connection;
use pinned_db_schema::{ models::{ NewPost, User }, schema::{ posts, users::{ self, token } } };
...
pub async fn create_new_post(
    ...
    match user {
        Ok(user) => {
            ...
            let insert_result = diesel
                ::insert_into(posts::table)
                .values(new_post)
                .get_result::<
                    (i32, String, String, String, String, i32, Vec<i32>, Vec<i32>, Vec<i32>)
                >(connection);
