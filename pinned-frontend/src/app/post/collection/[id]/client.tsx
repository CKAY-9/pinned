...
import { addCollaboratorToCollection, deleteCollection, getCollectionCollaborators, removeCollaboratorFromCollection } from "@/api/collections/collections.client";
...
const AddCollaborator = (props: {
	...
	const removeCollaborator = async (e: BaseSyntheticEvent, user_id: number) => {
		e.preventDefault();
		const remove = await removeCollaboratorFromCollection(props.collection.id, user_id);
		if (remove) {
				...
				{loading ? (
					...
					<>
						{users.map((user, index) => {
							return (
								<button key={index} style={{ "padding": "0" }} onClick={async (e: BaseSyntheticEvent) => await addCollaborator(e, user.id)}>
				...
				{existing_collabs.length <= 0 ? (
					...
					<>
						...
						{existing_collabs.map((user, index) => {
							return (
								<button key={index} style={{ "padding": "0" }} onClick={async (e: BaseSyntheticEvent) => await removeCollaborator(e, user.id)}>
									<UserPreview user={user}></UserPreview>
								</button>
							)
						})}
		...
		<>
			{show_add_collab && (
				<Popup>
					...
					<AddCollaborator collection={props.collection} />
				</Popup>
			)}
